...
import { addToAllNotifications, getNotificationsSize, getPagebleNotifications, setAllNotifications, setViewNotification } from "../../../../redux/notifications.slice/notifications.slice";
...
function HeaderNotifyOptions() {
    ...
    const unreadedNotificationsSize = useSelector(store => store.notifications.unreadedNotificationsSize);
    ...
    function handleScroll(e) {
        if (
            e.target.scrollHeight - (e.target.scrollTop + e.target.offsetHeight) < 100 && !isFetching
        ) {
            setIsFetching(true);
        }
    }
    ...
    useEffect(() => {
        if (newNotifications[0]) dispatch(addToAllNotifications(newNotifications[newNotifications.length - 1]));
    }, [newNotifications])
        ...
        <>
            <Tooltip title="Notifications" sx={{ p: { xs: "4px", sm: 1 } }}>
                <IconButton
                    ...
                    <Badge badgeContent={unreadedNotificationsSize + newNotifications.length} color="secondary">
                        <AvatarStyled>
                            <NotificationsIcon
                                style={{
