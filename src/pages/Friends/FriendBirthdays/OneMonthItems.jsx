...
function OneMonthItems(props) {
    ...
    const friendListBirthdays = ItemList.length > 0 
    ? ItemList.map(el => { 
        ...
        return <CardContentStyled key={el.id}> 
                ...
                {fullInfo && 
                    <Box sx={{ display: 'flex', gap: 1/2, height: 30, flexDirection: 'column', ml: 2}}>
                        ...
                        <BirthdayDate>
                            {(new Date(el.friend.birthDate)).toLocaleDateString('en-us', { day:"numeric", month:"long", year:"numeric" })}
                        </BirthdayDate>
                    </Box>
                }
                </CardContentStyled>;
